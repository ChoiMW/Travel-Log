...
### 5.6 후쿠오카/일본 대중교통 스마트 요금 계산 엔진 (Transit Fare Calculator)
- 후쿠오카 시영 지하철(공항선/나나쿠마선), 니시테츠 전철, 하카타 타비토 직행 버스, JR 특급(유후인노모리/소닉), 시내 택시 공식 운임 데이터베이스 탑재 (`fukuokaTransitFares.ts`).
- 승차역 ➔ 하차역 선택 시 공식 소요시간 및 정확한 엔화(JPY ¥) 요금 자동 산출.
- `[가계부에 바로 담기]` 클릭 시 여행 포켓 가계부 지출 항목(`expenses`)으로 즉시 원클릭 등록 및 1/N 정산 금액 자동 갱신.
...
- 국가별 국토 자산 정복률 프로그레스 바 (대한민국 250곳 / 일본 47개 도도부현).
...
- 시간순 여행 매거진 카드 타임라인.
- 여행 상세 모달: 일정표, 방문 지역 뱃지, 첨부 사진 그리드 갤러리, 만족도 별점, 메모.

### 4. 사진첩 (`PhotoGallery.tsx` & `PhotoLightbox.tsx`)
- 지역별 / 날짜별 필터링 지원 인피니티 갤러리.
...
- **[일정표 서브탭]**: Day-by-Day 일자별 시간순 방문 장소 관리 & 국내(네이버 지도), 일본(구글 맵스) 길찾기 1초 연동.
...
- **[가계부 서브탭]**: 지출 항목 관리, 1/N 인원수별 분담금 자동 계산, **후쿠오카 교통요금 스마트 계산기 연동 (엔화 ¥ / 원화 ₩)**.
