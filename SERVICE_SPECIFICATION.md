...
## 완벽 복원 및 재구현을 위한 마스터 시스템 명세서 (System Architecture & Implementation Guide)

본 문서는 **TravelLog Korea** 서비스의 기획 의도, UI/UX 설계, 시스템 아키텍처, 데이터 모델, 핵심 알고리즘, 컴포넌트 구조 및 구현 코드를 완벽하게 집대성한 마스터 가이드입니다. **본 문서 하나만으로 백엔드 없이 100% 동일한 서비스를 완벽하게 재구현할 수 있도록** 상세하게 기술되었습니다.
...
### 1.1 서비스 정의
**TravelLog Korea**는 대한민국 250개 시·군·구를 정복하고 기록하는 **100% 로컬 프라이빗(On-Device) 여행 라이프로그 슈퍼앱**입니다.
...
2. **Zero-Friction 여행 아카이빙 (EXIF 지오코딩)**:
   - 여행 사진을 드래그&드롭하면 사진 속 EXIF 메타데이터(촬영일시, GPS 위경도)를 자동 분석하여 250개 시·군·구 중 어디인지 1초 만에 판별하고 자동으로 여행 기록을 생성합니다.
3. **카토그래피 기반 2단계 계층형 정복 지도 (Hierarchical LOD Map)**:
   - 전국 줌아웃 시에는 17개 광역시·도 캡슐로 시원하게 표시되고, 클릭/줌인 시 250개 세부 시군구가 선명하게 나타나며 밀집 도시의 글자 겹침을 원천 차단합니다.
4. **여행 전주기 올인원 포켓 (Trip Pocket)**:
   - 여행 전(일정표 & 길찾기, 짐싸기 체크리스트, 사전 티켓/QR 저장) ➔ 여행 중(실시간 위치/날짜 기반 자동 매칭 활성화, 빠른 QR 제시) ➔ 여행 후(1/N 정산 가계부, AI 여행 DNA 리포트, 소셜 맵 카드 공유)를 여행 단위(1:1)로 완벽히 관리합니다.
...
│   ├── App.tsx                    # 전체 라우팅, 실시간 위치 매칭 배너, 글로벌 모달 상태 관리
│   ├── index.css                  # 디자인 시스템 (토스 블루 컬러 토큰, 다크모드, 공통 유틸리티)
│   ├── types/
│   │   └── travel.ts              # 전체 TypeScript 인터페이스 정의 (Trip, PhotoItem, ItineraryItem 등)
│   ├── db/
│   │   └── index.ts               # IndexedDB CRUD 래퍼 (trips, photos, app_settings 스토어)
│   ├── data/
│   │   └── koreaDistricts.json    # 전국 250개 시·군·구 SVG Path 및 폴리곤 중심 좌표 데이터
│   ├── utils/
│   │   ├── geoMatcher.ts          # GPS 위경도 ➔ 250개 시군구 매칭 및 검색 알고리즘
│   │   ├── exif.ts                # 사진 EXIF 메타데이터 파싱 및 썸네일 생성 유틸리티
│   │   ├── tripMatcher.ts         # 현재 위치(GPS) & 오늘 날짜 기반 여행 자동 매칭 엔진
